...
const valMax = 100;
const gradient = (value: [number] | number) => {
  const valueList = typeof value === 'number' ? [value] : value;
  const valueRange = valMax / colorList.length;
  const result = valueList.map(item => {
    return {
      value: item,
      valueRange,
      data: Math.floor(item / valueRange),
      color:
        colorList[
          Math.floor(item / valueRange) > colorList.length - 1
            ? colorList.length - 1
            : Math.floor(item / valueRange)
        ],
    };
  });
