...
function SS_Calendar(props: any) {
  ...
  const isToday = (sy: any, sm: any, sd: any) => {
    const Now = new Date();
    return Boolean(
      Now.getFullYear() === Number(sy) &&
        Now.getMonth() + 1 === Number(sm) &&
        Now.getDate() === Number(sd),
    );
  };
  ...
  const DateFullCellRender = (moment: any) => {
    ...
    return (
      <div className="ss-calendar-date">
        ...
        <div className="ss-calendar-content">{dateCellRender && dateCellRender(moment)}</div>
      </div>
    );
  };
  ...
  return (
    <Calendar
      className={classNames('ss-calendar', className, { 'hide-header': hideHeader })}
